...
def main():
    parser = argparse.ArgumentParser(description="指定した画像のコントラストを変更して保存するスクリプト")
    parser.add_argument("image_path", type=str, help="処理する画像のディレクトリまたはファイルのパス")
    parser.add_argument("--alpha", type=float, default=1.0, help="コントラストの変動倍率（1.0未満で低コントラスト、1.0より大きいと高コントラストになります。推奨: 0.5 ~ 2.0）")
    parser.add_argument("--output", type=str, default=None, help="保存先のファイルパス（指定しない場合は元のファイル名に '_alpha{値}' が付与されます）")
    ...
    def process_image(filepath: Path, output_path: Path = None):
        ...
        # コントラストの変更 (alphaを掛ける、beta=0で明るさはそのまま)
        adjusted_img = cv2.convertScaleAbs(img, alpha=args.alpha, beta=0)

        if output_path is None:
            # e2e_nav_box1/image ディレクトリに保存
            image_dir = Path(__file__).resolve().parent / "image"
            output_path = image_dir / f"{filepath.stem}_alpha{args.alpha}{filepath.suffix}"
        elif output_path.is_dir() or str(output_path).endswith('/') or not output_path.suffix:
            # outputがディレクトリとして指定された場合、ファイル名を付与
            output_path = output_path / f"{filepath.stem}_alpha{args.alpha}{filepath.suffix}"
